function new_baseline_spring
tic;                        %start timing

global sol sol1 SimValues1 u_ave;
...
t = linspace(99,99.5,3);        % definition of the time domain, t=[0 400] years with resolution of 401
...
SimValues1 = cell(NumVars,1);                      %creates an NumVars-by-1 cell array of empty matrices
                                                    %to store the concentration
u_ave = zeros(NumVars,511);
for i1 = 1:NumVars
    u_ave(i1,:) = sol(end,:,i1);
end

disp('solving spring ');
sol1 = pdepe(0,@pdex14pde,@pdex1ic,@pdex1bc,x,t);    %calls the solver and stores the result in 'sol'
toc;                                                %stop timing

for j = 1:NumVars
    SimValues1{j,1} = sol1(:,:,j);                     %store the solution in the cell,
...
w = 0.35; %0.23 or SAR/F;     %time-dependant burial rate
alfa0 = 0.10;                           %bioirrigation constant at sediment-water interface
