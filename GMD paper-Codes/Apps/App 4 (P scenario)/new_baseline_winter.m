function new_baseline_winter
tic;                        %start timing

global sol sol1 sol2 para;
global SimValues SimValues1 SimValues2 u_ox;
...
t = linspace(99.5,100,3);        % definition of the time domain, t=[0 400] years with resolution of 401
...
SimValues2 = cell(NumVars,1);                      %creates an NumVars-by-1 cell array of empty matrices
...
disp('solving winter ');
sol2 = pdepe(0,@pdex14pde,@pdex1ic,@pdex1bc,x,t);    %calls the solver and stores the result in 'sol'
toc;                                                %stop timing

for j = 1:NumVars
    SimValues2{j,1} = sol2(:,:,j);                     %store the solution in the cell,
end                                                 %cleaner as compared to the sol matrix

save('Result.mat','sol','sol1','sol2','SimValues','SimValues1','SimValues2','para');
...
k_psorb_a = 1e-2;               %PO4 sorption onto Fe(OH)3, R22
k_psorb_b = 1e-2;               %PO4 sorption onto FeOOH, R23
...
w = 0.23; %0.23 or SAR/F;     %time-dependant burial rate
